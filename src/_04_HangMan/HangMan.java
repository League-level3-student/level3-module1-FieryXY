...
public class HangMan{
	...
	static void newGame() {
		game = new HangMan(new JFrame(), new JPanel(), new JLabel());
		game.setup();
	}
	...
	void setup() {
		...
		try {
			...
			JOptionPane.showMessageDialog(frame, "There was a problem reading the number, so the game will only be 1 round");
		}
	...
	void manageGame() {
		...
		String playAgain = JOptionPane.showInputDialog("You won! Do you want to play again? Type Y for yes or N for no.");
	...
	void guessWord(String word) {
		...
		while(guessed == false) {
			...
			if(blankCount == 0) {
				guessed = true;
				JOptionPane.showMessageDialog(frame, "You did it! The word was: "+word+". Click OK to move on the next round.");
				break;
			}
			...
			char gC;
			 if(guessedCharStr.length() == 1) {
				...
				if(gC != ' ') {
					...
					JOptionPane.showMessageDialog(frame, "Invalid Letter. Try Again.");
				}
			}
			 else {
				 JOptionPane.showMessageDialog(frame, "Invalid Letter. Try Again.");
			...
			if(lives <= 0) {
				String playAgain = JOptionPane.showInputDialog("You Lost. Do you want to play again? Type Y for yes or N for no.");
				if(playAgain.equals("Y")) {
					System.out.println("Should play Again");
					newGame();
				}
